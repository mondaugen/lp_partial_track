...
def get_L_best_paths_mats(c,n_pts_per_frame,L):
    """
    Get the matrices for the quadratic program that finds the L best paths through
    a lattice and also returns each individual path.
    ...
    returns tuple (P,q,G,h,A,b)

    P:
        The matrix in the objective function of a quadratic program.
    q:  
        The vector in the objective function of a quadratic program.
    G:
...
L=5
(P,q,G,h,A,b,M)=get_L_best_paths_mats(c,n_pts_per_frame,L)
P_=cvxopt.sparse(cvxopt.matrix(P))
q_=cvxopt.matrix(q)
...
sol=cvxopt.solvers.qp(P_,q_,G_,h_,A=A_,b=b_,solver=opt_solver)
plt.clf()
